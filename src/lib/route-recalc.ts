 ...
 * 1) zo weinig mogelijk bezorgtijd-schendingen
 * 2) kortste totale rijtijd (geografie + depot-retouren)
 * 3) pas daarna load-balans (alleen bij gelijke scores)
 */
export async function chooseBestCapacityLegs(
  ...
  for (const c of unique) {
    try {
      ...
      const loads = c.legs.map((leg) => totalRouteStopLoad(leg));
      const imbalance =
      ...
      scored.push({ name: c.name, timed, violations, travelMin, imbalance });
    } catch (err) {
 ...
 * Respecteert bezorgtijd zo goed mogelijk (minst aantal schendingen), daarna rijtijd.
 *
 * `packMode: "full"` = altijd volle bus eerst.
 * Default `"best"` = minste bezorgtijd-schendingen, daarna kortste rijtijd.
 */
